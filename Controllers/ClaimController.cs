...
namespace Claiming_System.Controllers
{
    ...
    {
        private const string connectionString = "Server=localhost;Database=claimsystem;User=root;Password=;";
        ...
        public ActionResult Register(User user)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    conn.Open();
                    string query = "INSERT INTO user (fullname, email, password) VALUES (@FullName, @Email, @Password)";
        ...
        public ActionResult Login(string email, string password)
        {
            ...
            {
                conn.Open();
                string query = "SELECT * FROM user WHERE email = @Email AND password = @Password";  // Insecure: Implement password hashing in production
                using (var cmd = new MySqlCommand(query, conn))
                {
